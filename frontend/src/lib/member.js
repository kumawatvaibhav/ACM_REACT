const teamMembers = [
  {
    name: 'Ms. Mosam Patel',
    role: 'Faculty Sponsor',
    image: 'https://res.cloudinary.com/ddpmw1pgg/image/upload/v1734774283/Mosam_mam_qp9xhb.png',
    description: '',
  },
  {
    name: 'Nandish Mittal',
    role: 'Chair',
    image: 'https://res.cloudinary.com/ddpmw1pgg/image/upload/v1734774284/nandish_2_gdxwy6.jpg',
    description: '',
    github: 'https://github.com/Naadish-555',
    linkedin: 'https://www.linkedin.com/in/nandish-mittal/',
  },
  {
    name: 'Tejas Mistry',
    role: 'Vice Chair',
    image: 'https://res.cloudinary.com/ddpmw1pgg/image/upload/v1734774286/tejas_2_ito9pw.jpg',
    description: '',
    github: 'https://github.com/tejas2435',
    linkedin: 'https://www.linkedin.com/in/tejas-mistry-494621283',
  },
  {
    name: 'Kanika Sharma',
    role: 'Secretary',
    image: 'https://res.cloudinary.com/ddpmw1pgg/image/upload/v1734774284/Kanika_3_knm2bd.jpg',
    description: '',
  },
  {
    name: 'Vaibhav Kumawat',
    role: 'Web Master & Tech Lead',
    image: 'https://res.cloudinary.com/ddpmw1pgg/image/upload/v1734774284/vaibhav_tonlwh.png',
    description: '',
    github: 'https://github.com/kumawatvaibhav',
    linkedin: 'https://www.linkedin.com/in/vaibhav-kumawatt/'
  },
  {
    name: 'Chirag Parmer',
    role: 'Treasurer',
    image: 'https://res.cloudinary.com/ddpmw1pgg/image/upload/v1734774283/Chirag_2_lcwrvg.png',
    description: '',
  },
  {
    name: 'Shreya Dholiya',
    role: 'Membership Chair',
    image: 'https://res.cloudinary.com/ddpmw1pgg/image/upload/v1734774285/Shreya_2_hob7eq.jpg',
    description: '',
  },
  {
    name: 'Archit Pandey',
    role: 'Marketing & Sponsorship Head',
    image: 'https://res.cloudinary.com/ddpmw1pgg/image/upload/v1735458194/assets/officers/Team/msffwitnexyqre3gftqd.png',
    description: '',
    github: 'https://github.com/Architpandey01',
    linkedin: 'https://www.linkedin.com/in/architpandeyy/',
  },
  {
    name : 'Sameer Gaur',
    role : 'Content head',
    image : 'https://res.cloudinary.com/ddpmw1pgg/image/upload/v1738346462/assets/officers/Team/rhhxqpjf5jjohpibgqc5.jpg',
    description : '',
    github : 'https://github.com/Sameer-G30',
    linkedin : 'https://www.linkedin.com/in/sameer-gaur'
  },
  {
    name : 'Vidur Shah',
    role : 'Design Head',
    image : 'https://res.cloudinary.com/ddpmw1pgg/image/upload/v1738346000/assets/officers/Team/wu86msy4xtpefbaxsqpe.jpg',
    description : '',
    github : '',
    linkedin : 'https://www.linkedin.com/in/vidur-shah-ba6a0a283?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app'
  },
  {
    name : 'Divy Puranik',
    role : 'Logistics Head',
    image : 'https://res.cloudinary.com/ddpmw1pgg/image/upload/c_crop,w_2100,h_2800,ar_3:4,g_auto/v1735457908/assets/officers/Team/q6vf7jxuxorjcmk0qwp7.jpg',
    description : '', 
    github : 'https://github.com/DarkRising',
    linkedin : 'https://www.linkedin.com/in/divy-puranik/'
  },
  {
    name : 'Heer Modi',
    role : 'Event Head',
    image : 'https://res.cloudinary.com/ddpmw1pgg/image/upload/v1738991173/assets/officers/Team/mttxn8wtfgckrtqgt0ir.jpg',
    description : '', 
    github : 'https://github.com',
    linkedin : 'https://www.linkedin.com/in/heer-modi-0a4841256/'
  },
  {
    name : 'Tirth Sharma',
    role : 'Social Media Head',
    image : 'https://www.gravatar.com/avatar/?d=mp',
    description : '',
    github : 'https://github.com/Tirth9000',
    linkedin : 'https://www.linkedin.com/in/tirth-sharma-423867251?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=ios_app'
  }
];

export default teamMembers;